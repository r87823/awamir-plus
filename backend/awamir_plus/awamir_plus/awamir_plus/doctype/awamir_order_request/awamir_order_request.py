...
from awamir_plus.constants import ERP_SYNC_NOT_SYNCED, ORDER_STATUS_DRAFT
...
class AwamirOrderRequest(Document):
    ...
    def validate(self):
        ...
        self.remaining_amount = max(
            frappe.utils.flt(self.total_amount) + frappe.utils.flt(self.delivery_fee) - frappe.utils.flt(self.deposit_amount),
            0,
        )
