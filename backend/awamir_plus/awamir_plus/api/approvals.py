...
from awamir_plus.utils import assert_required, create_notification, get_users_with_role, make_status_log, now
...
def approve_order(order):
    ...
    if doc.status == ORDER_STATUS_SENT_TO_DISTRIBUTION:
        return _approval_response(doc, _("Order is already sent to distribution."))
    ...
    return _approval_response(doc, _("Order approved and sent to distribution."))
...
def reject_order(order, reason):
    ...
    if doc.status == ORDER_STATUS_REJECTED:
        return _approval_response(doc, _("Order is already rejected."))
    if doc.status != ORDER_STATUS_PENDING_APPROVAL:
    ...
    return _approval_response(doc, _("Order rejected."))


@frappe.whitelist()
def return_order_for_edit(order, notes=None, note=None):
    require_roles(["Awamir Branch Supervisor", "Awamir System Admin"])
    ...
    if doc.status == ORDER_STATUS_RETURNED:
        return _approval_response(doc, _("Order is already returned for edit."))
    if doc.status != ORDER_STATUS_PENDING_APPROVAL:
    ...
    return _approval_response(doc, _("Order returned for edit."))
